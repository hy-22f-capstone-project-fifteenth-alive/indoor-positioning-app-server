package net.jaram.indoornavigation.oauth.service;

import net.jaram.indoornavigation.oauth.domain.Member;
import net.jaram.indoornavigation.oauth.domain.OAuthAttributes;
import net.jaram.indoornavigation.oauth.domain.UserProfile;
import net.jaram.indoornavigation.oauth.repository.MemberRepository;
...
public class OAuthService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    private Member saveOrUpdate(UserProfile userProfile) {
        Member member = memberRepository.findByOauthId(userProfile.oauthId())
                .map(m -> m.update(userProfile.name(), userProfile.email(), userProfile.imageUrl())) // OAuth 서비스 사이트에서 유저 정보 변경이 있을 수 있기 때문에 우리 DB에도 update
